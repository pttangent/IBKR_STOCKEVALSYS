#!/usr/bin/env python3
"""Fetch bounded Yahoo/yfinance option-chain slices for evidence fallback."""
...
def fetch(symbol: str, spot: float, expiries_count: int, strikes_each_side: int, requested_expiries: list[str] | None = None) -> dict:
    try:
    ...
    for expiry in selected_expiries:
        try:
            ...
            audit.append({"expiry": expiry, "status": "ok", "calls": len(calls), "puts": len(puts), "selected_strikes": sorted(selected)})
    ...
    return {"provider": "yfinance", "retrieved_at": dt.datetime.now(dt.timezone.utc).isoformat(), "symbol": symbol.upper(),
            "status": status, "spot": spot, "request": {"expiries_count": expiries_count, "strikes_each_side": strikes_each_side},
...
def main():
    ...
    args = ap.parse_args()
    packet = fetch(args.symbol, args.spot, args.expiries_count, args.strikes_each_side, args.expiries)
    Path(args.out).write_text(json.dumps(packet, ensure_ascii=False, indent=2, default=str) + "\n", encoding="utf-8")
    print(json.dumps({"written": args.out, "symbol": packet["symbol"], "status": packet["status"], "options": len(packet["options"])}, ensure_ascii=False))
